/* Test: 00:0F:F0:12:C PORT 10, BATTERY: 3V
0x 00 05 04 68 2A 03 67 FF FF
o 0x 00 05 = Tag entry #5
 (Ambient RH) = (0x 2A) × 0.5% = 21%
 (Ambient Temperature) = (0x FF FF) × 0.1°C = −0.1°C
...
function parseUplink(device, payload)
{
   var payloadb = payload.asBytes(); 
    ...
    // Store temperature
    if (decoded.tagged_ambient_temperature != null) {
        var sensor1 = device.endpoints.byAddress("1");

        if (sensor1 != null)
            sensor1.updateTemperatureSensorStatus(decoded.tagged_ambient_temperature);

    };	
    // Store humidity
    if (decoded.tagged_relative_humidity != null) {
        var sensor2 = device.endpoints.byAddress("2");

        if (sensor2 != null)
            sensor2.updateHumiditySensorStatus(decoded.tagged_relative_humidity);

    // Store probe temperature
    /*if (decoded.tagged_ext_probe_temperature != null) {
        var sensor2 = device.endpoints.byAddress("4");

        if (sensor2 != null)
            sensor2.updateTemperatureSensorStatus(decoded.tagged_ext_probe_temperature);
    */
